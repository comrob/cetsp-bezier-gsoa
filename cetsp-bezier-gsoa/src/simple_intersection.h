...
namespace geom {
   ...
   /// ----------------------------------------------------------------------------
   template <class T, bool with_improper_intersection = false> class CIntersection
   {
      public:
         ...
         static TOrientation wind(const T &a, const T &b, const T &c)
         {
            ...
            return (w > .0001) ? LEFT_SIDE
               : ((w < -.0001) ? RIGHT_SIDE : COLLINEAR);
         }

         static double zeroRound(double x) { return fabs(x) < .00001 ? 0.0 : x; }
         static TOrientation windZeroRound(const T &a, const T &b, const T &c)
         {
            double w;
            w = (zeroRound(a.y - b.y) * zeroRound(c.x - b.x) -
                  zeroRound(c.y - b.y) * zeroRound(a.x - b.x));
            // need to allow for small math errors   seen with "gcc -O2 -mcpu=i686
            // -ffast-math"
            // return (w > .0001) ? 1 : ((w < -.0001) ? -1 : 0);
            return (w > .0001) ? LEFT_SIDE
      ...
      public:
         static bool Xor(bool x, bool y) { return !x ^ !y; }
               ...
               Int2Type<true>)
         {
            if (intersect(a, b, c, d, Int2Type<false>()))
               return true;
            else if (onSegment(a, b, c) || onSegment(a, b, d) ||
         ...
         /// ----------------------------------------------------------------------------
         static bool intersect(const T &a, const T &b, const T &c, const T &d,
               bool &improper, Int2Type<false>)
         {
            if (collinear(a, b, c) || collinear(a, b, d) || collinear(c, d, a) ||
                  collinear(c, d, b))
               return false;
            return Xor(wind(a, b, c) == LEFT_SIDE, wind(a, b, d) == LEFT_SIDE) &&
               Xor(wind(c, d, a) == LEFT_SIDE, wind(c, d, b) == LEFT_SIDE);
         }

         static bool intersect(const T &a, const T &b, const T &c, const T &d,
               bool &improper, Int2Type<true>)
         {
            if (intersect(a, b, c, d, Int2Type<false>()))
               return true;
            else if (onSegment(a, b, c) || onSegment(a, b, d) ||
         ...
         {
            improper = false;
            return intersect(a, b, c, d, improper,
                  Int2Type<with_improper_intersection>());
         }
         ...
         /// @param d
         ///
         /// @return true if segments (a,b) and (c,d) are collinear that is a,b,c and
         /// a,b,d are collinear
         /// ----------------------------------------------------------------------------
         ...
         /// ----------------------------------------------------------------------------
         static bool are_collinear_points_ordered_along_line(const T &a, const T &b,
               const T &c)
         {
         ...
         /// @param c
         ///
         /// @return true if a,b,c are collinear and c lies inside segment defined by
         /// a and ba
         /// TODO @deprecated
         /// ----------------------------------------------------------------------------
         ...
         ///
         /// @return true if point c lines on segment (a,b) same as above, but use
         /// already computed wind
         /// ----------------------------------------------------------------------------
         ...
         /// ----------------------------------------------------------------------------
         /// @brief bettwen based on algorithm ParallelInt from Computation Geomery
         /// in C
         ///
         /// @param a
         /// @param b
         ...
         /// ----------------------------------------------------------------------------
         /// @brief parallelIntersection based on algorithm ParallelInt from
         /// Computation Geomery in C
         ///
         /// @param a
         /// @param b
         /// @param c
         ...
         /// @return
         /// ----------------------------------------------------------------------------
         static char parallelIntersection(const T &a, const T &b, const T &c,
               const T &d, T &p)
         {
            if (!collinear(a, b, c)) {
               return '0';
         ...
         /// ----------------------------------------------------------------------------
         static char segmentIntersection(const T &a, const T &b, const T &c,
               const T &d, T &p)
         {
            static const double EPS = 0.00001;
            double s, t;
         ...
         static bool lineLineIntersect(const T &p1, const T &p2, const T &p3,
               const T &p4, T &pa, T &pb)
         {
            static const double EPS = 0.00001;
            T p13, p43, p21;
         ...
         /// ----------------------------------------------------------------------------
         /// @brief get_point_line_projections find point p as projection of c to
         /// line (a,b)
         ///
         /// @param c
         /// @param a
         ...
         /// @param p
         /// ----------------------------------------------------------------------------
         static void get_point_line_projections(const T &c, const T &a, const T &b,
               T &p)
         {
            const double r_num =
         ...
         /// @return
         /// ----------------------------------------------------------------------------
         static double point_line_squared_distance(const T &c, const T &a,
               const T &b)
         {
            const double r_den =
               (b.x - a.x) * (b.x - a.x) + (b.y - a.y) * (b.y - a.y);
         ...
         /// ----------------------------------------------------------------------------
         /// @brief point_segment_squared_distance return distance of point c from
         /// segment a,b
         ///
         /// @param c
         /// @param a
         ...
         /// @return
         /// ----------------------------------------------------------------------------
         static double point_segment_squared_distance(const T &c, const T &a,
               const T &b)
         {
            const double r_num =
               (c.x - a.x) * (b.x - a.x) + (c.y - a.y) * (b.y - a.y);
         ...
         ///
         /// same as above, but return indication endpoint indication and the point
         /// itself endpoint indication: 's' inside segment 'a' at the a point 'b' at
         /// the b point
         /// ----------------------------------------------------------------------------
         static double point_segment_squared_distance(const T &c, const T &a,
               const T &b, char &endpoint,
               T &p)
         {
            const double r_num =
               (c.x - a.x) * (b.x - a.x) + (c.y - a.y) * (b.y - a.y);
         ...
         static int cn_PnPoly(const T &P, const std::vector<T> &V)
         {
            ...
            for (int i = 0; i < n; i++) { // edge from V[i] to V[i+1]
               if (((V[i].y <= P.y) && (V[i + 1].y > P.y)) // an upward crossing
                  ...
                  double vt =
                     (double)1.0 * (P.y - V[i].y) / (V[i + 1].y - V[i].y);
                  if (P.x <
         ...
         // this seems be correct form the above listed version (be aware that V[n] =
         // V[0], only for the multiple component)
